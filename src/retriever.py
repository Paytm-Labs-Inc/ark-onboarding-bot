...
def build_index(chunks: list[Chunk], embeddings: np.ndarray | None = None) -> Index:
    ...
    and stored, which is the whole point of keeping the corpus in Postgres.
...
def load_default_index(data_dir: Path = DATA_DIR) -> Index:
    """The corpus, from Postgres when configured and from data/ otherwise.

    The stored path is not an optimisation with a fallback; it is the same
    index either way. Anything that stops Postgres answering -- unset,
    ...
    if load_stored_corpus is not None:
        ...
        if stored is not None:
            ...
            print(
                f"corpus loaded from Postgres: {len(chunks)} chunks, no embedding at boot"
            )
            _default_index_source = "postgres"
...
def default_index_info() -> dict[str, object]:
    ...
    served rows from Postgres, and would fail outright on an image that no
