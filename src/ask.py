...
def _new_request_id() -> str:
    """Short id joining this answer to the access log and any error line."""
...
def _log_ask_result(
    ...
    *,
    ...
    try:
        ...
        # The answer is already generated by this point. A full disk, a
        # read-only filesystem, or a serialisation bug in the record are all
        # logging problems, not answering problems.
        print(f"query log write failed: {type(exc).__name__}: {exc}", flush=True)
        ...
        if event.get("type") == "done":
            ...
            if log:
                _log_ask_result(
                    ...
                    duration_ms=_elapsed_ms(started), ttft_ms=ttft_ms,
                    request_id=request_id,
                )
            yield result
        else:
            # The first non-done event is the first token reaching the user --
            # the number streaming exists to improve, and the one they feel.
            if ttft_ms is None:
    ...
    if cached is not None:
        if log:
            _log_ask_result(
                ...
                cache_hit=True, duration_ms=_elapsed_ms(started),
                request_id=request_id,
            )
        return cached
...
def run_question(
    ...
    *,
    ...
    if cached is not None:
        _log_ask_result(
            question, cached, channel=channel, session_id=session_id,
            cache_hit=True, duration_ms=_elapsed_ms(started), request_id=request_id,
        )
        return cached
